...
if ($user['role'] === 'superadmin' && ($_SESSION['role'] ?? '') !== 'superadmin') {
    die("Access Denied: You cannot edit a Superadmin user.");
}
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <form method="POST">
                    ...
                    <?php if ($user_id == $_SESSION['user_id']): ?>
                        ...
                        <select disabled style="background: #333; color: #aaa;">
                            <option value="admin" <?php if ($user['role'] == 'admin')
                                echo 'selected'; ?>>Admin (Full Access)
                            </option>
                            <option value="user" <?php if ($user['role'] == 'user')
                                echo 'selected'; ?>>Full User (All Tags)
                            </option>
                            <option value="tag_editor" <?php if ($user['role'] == 'tag_editor')
                                echo 'selected'; ?>>Tag Editor
                                (Restricted)</option>
                        </select>
                ...
                <div id="tag-selection"
                    style="display: <?php echo ($user['role'] == 'tag_editor') ? 'block' : 'none'; ?>; margin-bottom: 1em; padding: 1em; background: #2c2c2c; border-radius: 4px;">
                    ...
                    <div
                        ...
                        <?php foreach ($all_tags as $tag): ?>
                            <label style="display: flex; align-items: center; gap: 5px; font-weight: normal; color: #fff;">
                                <input type="checkbox" name="tags[]" value="<?php echo $tag['id']; ?>" <?php if (in_array($tag['id'], $assigned_tags))
                                       echo 'checked'; ?> style="width: auto;">
                ...
                <div class="form-group">
                    <label style="display: flex; align-items: center; gap: 10px;">
                        <input type="checkbox" name="can_change_password" value="1" <?php if ($user['can_change_password'])
                            echo 'checked'; ?> style="width: auto;">
        ...
        <div class="card">
            ...
            <p><strong>2FA Status:</strong>
                <?php echo !empty($user['totp_secret']) ? '<span style="color:var(--success-color);">Enabled</span>' : '<span style="color:#777;">Disabled</span>'; ?>
            </p>
